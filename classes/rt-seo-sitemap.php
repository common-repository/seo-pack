...
class RT_SEO_Sitemap {
	...
	function generate() {
		global $rtsp_options;
		if ( $rtsp_options['rtseo_mobile_sitemap'] ) {
...
*/
		...
		$this->home( $rtsp_options['rtseo_exclude_pages'] );

		$this->blog( $rtsp_options['rtseo_exclude_pages'] );

		add_filter( 'posts_where_request', 'rtsp_kill_query' );

		$this->pages( $rtsp_options['rtseo_exclude_pages'] );

		$this->posts( 'post', true, $rtsp_options['rtseo_exclude_pages'] );

		if ( $rtsp_options['rtseo_inc_custom_posts'] ) {
			$this->custom_posts( '', $rtsp_options['rtseo_exclude_pages'] );
		}

		if ( $rtsp_options['rtseo_inc_categories'] ) {
			$this->categories();
		}

		if ( $rtsp_options['rtseo_inc_tags'] ) {
			$this->tags();
		}

		if ( $rtsp_options['rtseo_inc_authors'] ) {
			$this->authors( $rtsp_options['rtseo_empty_author'] );
		}

		if ( $rtsp_options['rtseo_inc_archives'] ) {
			$this->archives();
		}

		remove_filter( 'posts_where_request', 'rtsp_kill_query');
	...
	function open() {
		...
		$o = '<?xml version="1.0" encoding="UTF-8"?>' . "\n"
		     . '<!-- Sitemap Generator Realtime SEO Pack -->' . "\n"
